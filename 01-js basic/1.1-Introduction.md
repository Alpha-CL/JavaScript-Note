...
* 在JavaScript中，标识符用来命名变量（和关键字，功能和标签）。

* 法律名称的规则更加大多数编程语言一样。

* 在JavaScript中，第一个字符必须是字母或下划线（_）或一个美元符号（$）。

* 接下来的字符可以是字母，数字，下划线或美元符号

> 所有的 JavaScript 名称的 `区分大小写` ，
> `-`连字符不能在 JavaScript中是不允许的，它们被保留用于减法 
